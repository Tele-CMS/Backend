...
namespace HC.Patient.Service.PatientApp
{
    public class PatientAppointmentService : BaseService, IPatientAppointmentService
    {
        ...
        private readonly object appointmentStatusModel;
        ...
        public JsonModel GetPatientAppointmentListForDashboard(string locationIds, string staffIds, string patientIds, DateTime? fromDate, DateTime? toDate, string patientTags, string staffTags, TokenModel token)
        {
            try
            {
                ...
                if (isValid)
                    //list = _appointmentRepository.GetAppointmentList<PatientAppointmentModel>(locationIds, staffIds, patientIds, fromDate, toDate, patientTags, staffTags, token.OrganizationID).ToList();
                    list = _appointmentRepository.GetAppointmentListForProviderDashboard<PatientAppointmentModel>(locationIds, staffIds, patientIds, fromDate, toDate, patientTags, staffTags, token.OrganizationID).ToList();
                return response = new JsonModel()
                {
                    data = list,
                    Message = StatusMessage.FetchMessage,
        ...
        public JsonModel GetAppointmentDetails(int appointmentId, TokenModel token, bool isGetPatientDetails = false)
        {
            try
            {
                ...
                if (isGetPatientDetails && pat.PatientID != null)
                {
                    int patientId = Convert.ToInt32(pat.PatientID);
                    var patient = _patientService.GetPatientsDetails(patientId, token);
                    patientAppointmentModel.PatientInfoDetails = (PatientInfoDetails)patient.data;
        ...
        public JsonModel CancelAppointments(int[] appointmentIds, int CancelTypeId, string reson, TokenModel token)
        {
            try
            {
                var isRequestedUserPatient = _context.Patients.Any(x => x.UserID == token.UserID);
                if (!isRequestedUserPatient)
                {
                    var CancelType = _context.GlobalCode.FirstOrDefault(x => x.Id == CancelTypeId);
                    if (CancelType.GlobalCodeValue == "Cancel By Client")
                        isRequestedUserPatient = true;
                }
                ...
                if (result > 0)
                {
                    ...
                    for (var i = 0; i < appointmentIds.Length; i++)
                    {
                        ...
                        if (appPayment != null)
                        {
                            ...
                            if (isRequestedUserPatient)
                            {
                                var appointmentStaff = _context.AppointmentStaff.FirstOrDefault(x => x.PatientAppointmentID == appointmentIds[i]);
                                if (appointmentStaff != null)
                                {
                                    var refundRules = _staffRepository.GetStaffsFeesAndRefundRules(new List<int> { appointmentStaff.StaffID });
                                    if (refundRules.Count() > 0)
                                    {
                                        var rules = refundRules.FirstOrDefault(x => x.CancelationRules.Count() > 0);
                                        if (rules != null)
                                        {

                                            var appointment = dbAppointments.FirstOrDefault(x => x.Id == appointmentIds[i]);
                                            var diffHours = (appointment.StartDateTime - DateTime.Now).TotalHours;
                                            if (diffHours > 0)
                                            {
                                                var cancelationRules = rules.CancelationRules.OrderByDescending(x => x.UptoHours);
                                                var maxUptoHourRule = cancelationRules.FirstOrDefault();
                                                if (maxUptoHourRule.UptoHours < diffHours)
                                                {
                                                    _refundAmount = appPayment.BookingAmount * 100 * maxUptoHourRule.RefundPercentage / 100;
                                                }
                                                else
                                                {
                                                    var toUseRules = cancelationRules.Where(x => x.UptoHours >= diffHours);
                                                    var refundRule = toUseRules.OrderByDescending(x => x.UptoHours).FirstOrDefault();
                                                    if (refundRule != null)
                                                    {
                                                        _refundAmount = appPayment.BookingAmount * 100 * refundRule.RefundPercentage / 100;
                                                    }
                                                }
                            ...
                            if (!string.IsNullOrEmpty(appPayment.PaymentToken))
                            {
                                StripeConfiguration.ApiKey = org.StripeSecretKey;
                                var options = new RefundCreateOptions();
                                options.Charge = appPayment.PaymentToken;
                                if (_refundAmount != 0)
                                {
                                    options.Amount = Convert.ToInt64(_refundAmount);
                                }
                    ...
                    this.SendCancelationAppointmentNotificationToSubscribers(dbAppointments, token);
        ...
        private void SendCancelationAppointmentNotificationToSubscribers(List<PatientAppointment> canceledAppointments, TokenModel token)
        {
            foreach (PatientAppointment oldappointment in canceledAppointments)
            {
                List<PatientAppointment> dbAppointments = _appointmentRepository.GetAll(a => a.IsNotify == true && a.StartDateTime >= oldappointment.StartDateTime).ToList();
                foreach (PatientAppointment selectedpatient in dbAppointments)
                {
                    var FrontendServer = _configuration.GetSection("FrontendServer").Value;
                    string message = $"Hi Sir/Ma'am, Hope you are doing well. There are new slot available please check at Date:{oldappointment.StartDateTime}. Click here to book now {FrontendServer + "web/waiting-room/reshedule/" + selectedpatient.Id}";
                    int patientId = (int)selectedpatient.PatientID;// (int)dbAppointments.Select(x => x.PatientID).FirstOrDefault();
                                                                   //sending Email
                    var patientResponse = _patientService.GetPatientById((int)patientId, token);
                    PatientDemographicsModel patient;
                    if (patientResponse.StatusCode == (int)HttpStatusCodes.OK)
                        patient = (PatientDemographicsModel)patientResponse.data;
                    else
                    ...
                    if (patient != null)
                    {
                        SendAppointmentNotificationEmailOnSlotFree(oldappointment,
                           selectedpatient.Id,
                           patient.Email,
                           patient.FirstName + " " + patient.LastName,
                           (int)EmailType.BookAppointment,
                           (int)EmailSubType.RejectApointment,
                           selectedpatient.Id,
                           "/templates/appointment-notifcation-onslot-available.html",
                           "Appointment Notification For Slot Availability",
                           token,
                           token.Request.Request
                           );
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(deviceToken))
                    {
                        PushMobileNotificationModel pushMobileNotification = new PushMobileNotificationModel();
                        pushMobileNotification.DeviceToken = deviceToken;
                        pushMobileNotification.Message = $"Hi Sir/Ma'am, Hope you are doing well. There are new slot available please check at Date:{oldappointment.StartDateTime}";
                        pushMobileNotification.NotificationPriority = PushNotificationPriority.High;
                        pushMobileNotification.NotificationType = CommonEnum.NotificationActionType.RejectAppointment.ToString();
                        PushNotificationsUserDetailsModel model = new PushNotificationsUserDetailsModel()
                        {
                            ProviderID = staffId,
                            PatientID = (int)selectedpatient.PatientID,// .Select(x => x.PatientID).FirstOrDefault(),
                            AppointmentId = selectedpatient.Id,// dbAppointments.Select(x => x.Id).FirstOrDefault(),
                            ImageThumbnail = "",// string.IsNullOrEmpty(staffs.PhotoThumbnailPath) ? staffs.PhotoThumbnailPath : CommonMethods.CreateImageUrl(token.Request, ImagesPath.StaffThumbPhotos, staffs.PhotoThumbnailPath),
                            Name = "",
                            Address = "",
                            StartDate = selectedpatient.StartDateTime.ToString("yyyy-MM-ddTHH:mm:ss"),
                            EndDate = selectedpatient.EndDateTime.ToString("yyyy-MM-ddTHH:mm:ss"),
                            StatusName = "NewSlotAvailable" //selectedpatient.AppointmentStatus.GlobalCodeName// dbAppointments.Select(x => x.AppointmentStatus.GlobalCodeName).FirstOrDefault(),
                        };
                        pushMobileNotification.Data = model;
                        PushNotificationsService.SendPushNotificationForMobile(pushMobileNotification);
                    }
        ...
        public JsonModel ApproveAppointment(AppointmentStatusModel appointmentStatusModel, TokenModel token)
        {
            response = new JsonModel(new object(), StatusMessage.ServerError, (int)HttpStatusCodes.InternalServerError, string.Empty);
            List<AppointmentAuthModel> list = null;
            PatientAppointment patientAppointment = null;
            ...
            if (patientAppointment != null)
            {
                ...
                //        else
                //            patientAppointment.StatusId = _globalCodeService.GetGlobalCodeValueId(GlobalCodeName.AppointmentStatus, appointmentStatusModel.Status, token);
                //        patientAppointment.AppointmentStaff = null;
                //        patientAppointment.AppointmentStatus = null;
                //        //patientAppointment.Patient = null;
                //        patientAppointment.UpdatedBy = token.UserID;
                //        patientAppointment.UpdatedDate = DateTime.UtcNow;
                //        _appointmentRepository.Update(patientAppointment);
                //        _appointmentRepository.SaveChanges();
                ...
                if (response.StatusCode == (int)HttpStatusCodes.OK)
                {
                    if (appointmentStatusModel.Status.ToLower() == AppointmentStatus.APPROVED.ToLower())
                    {
                        ...
                        string message = _notificationRepository.GetNotificationMessage(null, staffId, appointmentStatusModel.Id, "approved", locationModal);
                        Staffs staffs = _staffRepository.Get(a => a.Id == staffId && a.IsDeleted == false && a.IsActive == true);
                        patientAppointment = _appointmentRepository.Get(a => a.Id == appointmentStatusModel.Id);
                        var providerName = staffs.FirstName + " " + staffs.LastName;
                        PushNotificationModel saveNotificationModel = new PushNotificationModel()
        ...
        public JsonModel BookNewAppointementFromPatientPortal(PatientAppointmentModel patientAppointmentModel, TokenModel token)
        {
            ...
            if (response.StatusCode == (int)HttpStatusCodes.OK)
            {
                ...
                var patientResponse = _patientService.GetPatientById(patientAppointmentModel.PatientID != null && Convert.ToInt32(patientAppointmentModel.PatientID) != 0 ? Convert.ToInt32(patientAppointmentModel.PatientID) : token.StaffID, token);
                PatientDemographicsModel patient;
                if (patientResponse.StatusCode == (int)HttpStatusCodes.OK)
                    patient = (PatientDemographicsModel)patientResponse.data;
                else
                ...
                if (patient != null)
                {
                    ...
                    //string apptpayment = SendAppointmentPaymentEmail(patientAppointmentModel,
                    //    patient.Email,
                    //     patient.FirstName + " " + patient.LastName,
                    //     (int)EmailType.BookAppointment,
                    //     (int)EmailSubType.BookAppointmentToClient,
                    //     appointmentId,
                    //     "/templates/book-appointment-payment.html",
                    //     "Appoointment Booked Successfully",
                    //     token,
                    //     token.Request.Request
                    //     );
                ...
                AppointmentStatusModel.Id = appointmentId;
                AppointmentStatusModel.Status = patientAppointmentModel.StatusId == null ? "APPROVED" : "PENDING";
                ApproveAppointment(AppointmentStatusModel, token);
                ...
                string message = _notificationRepository.GetNotificationMessage(fullName, null, appointmentId, "requested", locationModal);
                PushNotificationModel saveNotificationModel = new PushNotificationModel()
                {
                    Message = message,
                    NotificationTypeId = (int)NotificationType.PushNotification,
                ...
                if (patientAppointmentModel != null && patientAppointmentModel.PatientID != 0 && patient != null && patientAppointmentModel.StatusId != null)
                {


                    SendAppointmentPaymentEmail(patientAppointmentModel, patient.Email, patient.FirstName + " " + patient.LastName, (int)EmailType.BookAppointment, (int)EmailSubType.BookAppointmentToProvider, appointmentId, "/templates/book-appointment-payment.html", "New Appoointment Booked", token, token.Request.Request);
        ...
        private string SendAppointmentNotificationEmailOnSlotFree(PatientAppointment patientAppointmentModel,int appointmentid, string toEmail, string username, int emailType, int emailSubType, int primaryId, string templatePath, string subject, TokenModel tokenModel, HttpRequest Request)
        {
            ...
            var hostingServer = _configuration.GetSection("HostingServer").Value;
            var FrontendServer = _configuration.GetSection("FrontendServer").Value;
            emailHtml = emailHtml.Replace("{{click_url}}", FrontendServer + "web/waiting-room/reshedule/" + appointmentid);
        ...
        private string SendAppointmentPaymentEmail(PatientAppointmentModel patientAppointmentModel, string toEmail, string username, int emailType, int emailSubType, int primaryId, string templatePath, string subject, TokenModel tokenModel, HttpRequest Request)
        {
            //Get Current Login User Organization
            tokenModel.Request.Request.Headers.TryGetValue("BusinessToken", out StringValues businessName);
            Organization organization = _tokenService.GetOrganizationByOrgId(tokenModel.OrganizationID, tokenModel);
            ...
            //#endif
            var hostingServer = _configuration.GetSection("HostingServer").Value;
            var FrontendServerurl = _configuration.GetSection("FrontendServer").Value;
            //string urlpath = "https://localhost:4200/web/new-payment";
            string urlpath = FrontendServerurl + "web/new-payment";
            emailHtml = emailHtml.Replace("{{img_url}}", hostingServer + "img/cbimage.jpg");
            emailHtml = emailHtml.Replace("{{username}}", username);
            emailHtml = emailHtml.Replace("{{operating_system}}", osNameAndVersion);
            emailHtml = emailHtml.Replace("{{browser_name}}", Request.Headers["User-Agent"].ToString());
            ...
            emailHtml = emailHtml.Replace("{{AppMode}}", patientAppointmentModel.Mode);
            emailHtml = emailHtml.Replace("{{AppType}}", patientAppointmentModel.Type);
            emailHtml = emailHtml.Replace("{{action_url}}", urlpath + "" + "?apptid=" + primaryId.ToString() + "&amt=" + patientAppointmentModel.PayRate);
            EmailModel emailModel = new EmailModel
            {
                EmailBody = CommonMethods.Encrypt(emailHtml),
                ToEmail = CommonMethods.Encrypt(toEmail),
        ...
        private string SendAppointmentEmailAfterPaymentUpdate(PatientAppointmentModel patientAppointmentModel, string toEmail, string username, int emailType, int emailSubType, int primaryId, string templatePath, string subject, TokenModel tokenModel, HttpRequest Request)
        {
            //Get Current Login User Organization
            tokenModel.Request.Request.Headers.TryGetValue("BusinessToken", out StringValues businessName);
            Organization organization = _tokenService.GetOrganizationByOrgId(tokenModel.OrganizationID, tokenModel);
            ...
            EmailModel emailModel = new EmailModel
            {
                ...
                EmailSubType = emailSubType,
                PrimaryId = primaryId,
                CreatedBy = 6//tokenModel.UserID == 0 ? patientAppointmentModel.ReportId : tokenModel.UserID
            };
            ////Send Email
            ////await _emailSender.SendEmailAsync(userInvitationModel.Email, string.Format("Invitation From {0}", orgData.OrganizationName), emailHtml, organizationSMTPDetailModel, orgData.OrganizationName);
            //var isEmailSent = _emailSender.SendEmails(toEmail, subject, emailHtml, organizationSMTPDetailModel, organization.OrganizationName).Result; //_emailSender.SendEmail(organizationSMTPDetailModel.SMTPUserName, subject, emailHtml, organizationSMTPDetailModel, organization.OrganizationName, toEmail);
            ...
            using (IDbContextTransaction transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (patientAppointmentModel.PatientAppointmentId == 0)
                    {
                        ...
                        patientAppointment.StatusId = _globalCodeService.GetGlobalCodeValueId(GlobalCodeName.AppointmentStatus, patientAppointment.StatusId == null ? AppointmentStatus.APPROVED : AppointmentStatus.PENDING, token);
                        ...
                        patientAppointment.IsNotify = patientAppointmentModel.IsNotify;
                        patientAppointment.IsUrgentCareAppointment = patientAppointmentModel.IsUrgentCareAppointment;
                        _appointmentRepository.Create(patientAppointment);
                        _appointmentRepository.SaveChanges();
        ...
        public JsonModel RescheduleAppointment(RescheduleModel rescheduleModel, TokenModel token)
        {
            var pat = _context.PatientAppointment
                .Include(a => a.AppointmentStaff)
                .Include(a => a.AppointmentStaff).ThenInclude(t => t.Staffs)
                .Where(x => x.Id == rescheduleModel.AppointmentId).FirstOrDefault();
            int timeinterVal = 30;
            LocationModel locationModal = _locationService.GetLocationOffsets(pat.ServiceLocationID, token);
            var staff = (pat.AppointmentStaff != null && pat.AppointmentStaff.Count() > 0) ? pat.AppointmentStaff.FirstOrDefault().Staffs : null;


            if (staff != null && staff.TimeInterval != 0 && staff.TimeInterval != null)
            {
                timeinterVal = Convert.ToInt32(staff.TimeInterval);
            }
            rescheduleModel.EndDateTime = rescheduleModel.StartDateTime.AddMinutes(timeinterVal);

            PatientAppointment patientAppointment = _appointmentRepository.Get(x => x.Id == rescheduleModel.AppointmentId && x.IsActive == true && x.IsDeleted == false);
            if (!ReferenceEquals(patientAppointment, null))
            {
                patientAppointment.StartDateTime = CommonMethods.ConvertToUtcTimeWithOffset(rescheduleModel.StartDateTime, locationModal.DaylightOffset, locationModal.StandardOffset, locationModal.TimeZoneName);
                patientAppointment.EndDateTime = CommonMethods.ConvertToUtcTimeWithOffset(rescheduleModel.EndDateTime, locationModal.DaylightOffset, locationModal.StandardOffset, locationModal.TimeZoneName);
                ///patientAppointment.AppointmentTypeID = patientAppointmentModel.AppointmentTypeID;
                patientAppointment.AppointmentTypeID = null;
                patientAppointment.Notes = patientAppointment.Notes;
                patientAppointment.PatientAddressID = patientAppointment.PatientAddressID;
                patientAppointment.ServiceLocationID = patientAppointment.ServiceLocationID;
                patientAppointment.OfficeAddressID = patientAppointment.OfficeAddressID;
                patientAppointment.CustomAddress = patientAppointment.CustomAddress;
                patientAppointment.CustomAddressID = patientAppointment.CustomAddressID;
                patientAppointment.Longitude = patientAppointment.Longitude;
                patientAppointment.Latitude = patientAppointment.Latitude;
                patientAppointment.ApartmentNumber = patientAppointment.ApartmentNumber;
                //patientAppointment.IsDirectService = patientAppointmentModel.IsDirectService;
                patientAppointment.IsTelehealthAppointment = true;
                patientAppointment.IsDirectService = true;
                patientAppointment.IsExcludedFromMileage = true;
                ...
                patientAppointment.UpdatedBy = token.UserID;
                patientAppointment.UpdatedDate = DateTime.UtcNow;
                patientAppointment.Offset = (int)CommonMethods.GetCurrentOffset(rescheduleModel.StartDateTime, locationModal.DaylightOffset, locationModal.StandardOffset, locationModal.TimeZoneName);
                ...
                _appointmentRepository.Update(patientAppointment);
                _appointmentRepository.SaveChanges();
                string message = _notificationRepository.GetNotificationMessage(null, patientAppointment.StaffIDs.Length > 0 ? patientAppointment.StaffIDs[0] : 0, patientAppointment.Id, "rescheduled");
                PushNotificationModel saveNotificationModel = new PushNotificationModel()
                {
                    //Message = NotificationMessage.AppointmentAccepted + " " + providerName,
                    Message = message,
                ...
                response = new JsonModel(null, StatusMessage.UpdateAppointment, (int)HttpStatusCodes.OK, string.Empty);
            }
            return response;
        }
        ...
        public JsonModel GetPendingAppointmentList(PatientAppointmentFilterModel appointmentFilterModel, TokenModel token)
        {
            ...
            if (list != null && list.Count() > 0)
            {
                foreach (var x in list)
                {
                    ...
                    if (!string.IsNullOrEmpty(x.PhotoThumbnailPath)) { x.PhotoThumbnailPath = CommonMethods.CreateImageUrl(token.Request, ImagesPath.PatientThumbPhotos, x.PhotoThumbnailPath); }
        ...
        public JsonModel BookNewFreeAppointementFromPatientPortal(PatientAppointmentModel patientAppointmentModel, TokenModel token)
        {
            ...
            if (response.StatusCode == (int)HttpStatusCodes.OK)
            {
                ...
                AppointmentStatusModel AppointmentStatusModel = new AppointmentStatusModel();
                AppointmentStatusModel.Id = appointmentId;
                AppointmentStatusModel.Status = "APPROVED";
                ApproveAppointment(AppointmentStatusModel, token);
        ...
        public JsonModel GetLastNewAppintment(int providerId, TokenModel token)
        {
            response = new JsonModel();
            try
            {
                bool isAlloed = false;
                List<PatientAppointment> appts = new List<PatientAppointment>();
                var latestNewAppointments = _patientAppointmentRepository.GetLastNewAppintment(providerId, token);
                if (latestNewAppointments.Count > 0)
                {
                    foreach (var a in latestNewAppointments)
                    {
                        LocationModel loggedInlocation = _locationService.GetLocationOffsets(a.ServiceLocationID, token);
                        a.StartDateTime = CommonMethods.ConvertFromUtcTimeWithOffset(a.StartDateTime, loggedInlocation.DaylightOffset, loggedInlocation.StandardOffset, loggedInlocation.TimeZoneName, token);
                        appts.Add(a);
                    }
                }

                var latestAppt = appts.Where(x => x.StartDateTime < DateTime.Now).OrderByDescending(x => x.StartDateTime).FirstOrDefault();
                var staff = _context.Staffs.FirstOrDefault(x => x.Id == providerId);
                if (staff != null && latestAppt != null)
                {
                    var followUpDays = Convert.ToInt32(staff.FollowUpDays);
                    if (followUpDays > 0)
                    {
                        var timeAllowed = latestAppt.StartDateTime.AddDays(followUpDays);
                        if (latestAppt.StartDateTime <= timeAllowed)
                        {
                            isAlloed = true;
                        }
                    }
                ...
                response.data = isAlloed ? latestAppt : null;
        ...
        public JsonModel GetPreviousAppintment(int providerId, int clientId, TokenModel token)
        {
            response = new JsonModel();
            try
            {
                bool isAlloed = false;
                List<PatientAppointment> appts = new List<PatientAppointment>();
                var latestNewAppointments = _patientAppointmentRepository.GetPreviousAppintment(providerId, clientId, token);
                if (latestNewAppointments != null && latestNewAppointments.Count > 0)
                {
                    foreach (var a in latestNewAppointments)
                    {
                        LocationModel loggedInlocation = _locationService.GetLocationOffsets(a.ServiceLocationID, token);
        ...
        public JsonModel BookNewAppointmentFromPaymentPage(PatientAppointmentModel patientAppointmentModel, TokenModel token)
        {
            JsonModel response = UpdatePaymentForAppointment(patientAppointmentModel, token);
            if (response.StatusCode == (int)HttpStatusCodes.OK)
            {
                int.TryParse(response.data.ToString(), out int appointmentId);
                ...
                PatientAppointment patientAppointment = null;
                AppointmentStaff appointmentStaff = null;
                Patients patientss = null;
                User userdetails = null;

                patientAppointment = _appointmentRepository.Get(x => x.Id == patientAppointmentModel.PatientAppointmentId && x.IsActive == true && x.IsDeleted == false);
                appointmentStaff = _appointmentStaffRepository.GetAll(x => x.PatientAppointmentID == patientAppointmentModel.PatientAppointmentId && x.IsActive == true && x.IsDeleted == false).FirstOrDefault();
                patientss = _context.Patients.Where(x => x.Id == patientAppointment.PatientID && x.IsActive == true && x.IsDeleted == false).FirstOrDefault();
                userdetails = _context.User.Where(x => x.Id == patientss.UserID && x.IsActive == true && x.IsDeleted == false).FirstOrDefault();

                var staffResponse = _staffService.GetStaffProfileData(appointmentStaff.StaffID, token);
                StaffProfileModel staff;
                ...
                var patientResponse = _patientService.GetPatientByIdForPushNotificatons(patientAppointment.PatientID, token);
                PatientDemographicsModel patient;
                if (patientResponse.StatusCode == (int)HttpStatusCodes.OK)
                    patient = (PatientDemographicsModel)patientResponse.data;
                else
                ...
                patient.Email = patient.Email == null ? userdetails.UserName : patient.Email;
                patientAppointmentModel.Mode = patientAppointment.BookingMode;
                patientAppointmentModel.Type = patientAppointment.BookingType;
                patientAppointmentModel.StartDateTime = patientAppointment.StartDateTime;
                patientAppointmentModel.EndDateTime = patientAppointment.EndDateTime;

                if (staff != null)
                {
                    string error = SendAppointmentEmailAfterPaymentUpdate(patientAppointmentModel,
                        staff.Email,
                         staff.FirstName + " " + staff.LastName,
                         (int)EmailType.BookAppointment,
                         (int)EmailSubType.BookAppointmentToProvider,
        ...
        public JsonModel UpdatePaymentForAppointment(PatientAppointmentModel patientAppointmentModel, TokenModel token)
        {

            var organizationJsonModel = _organizationService.GetOrganizationDetailsById(token);
            //LocationModel locationModal = _locationService.GetLocationOffsets(patientAppointmentModel.ServiceLocationID, token);
            Updatestatusforappointment(patientAppointmentModel, token);
            using (IDbContextTransaction transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    //if (!ReferenceEquals(patientAppointment, null))
                    //{
                    //    //patientAppointment.StatusId = _globalCodeService.GetGlobalCodeValueId(GlobalCodeName.AppointmentStatus, AppointmentStatus.APPROVED, token);
                    ...
                    //    //response = new JsonModel() { Message = StatusMessage.UpdatePatientAppointment , data = appointmentStaff.StaffID};
                    //    response = new JsonModel(patientAppointment.Id, StatusMessage.UpdatePatientAppointment, (int)HttpStatusCodes.OK, string.Empty);
                    ...
                    if (response.StatusCode == (int)HttpStatusCodes.OK && !string.IsNullOrEmpty(patientAppointmentModel.PaymentToken))
                    {
                        ...
                        patient = _context.Patients.Where(x => x.Id == patientAppointment.PatientID && x.IsActive == true && x.IsDeleted == false).FirstOrDefault();
                        var appointmentPayment = _mapper.Map<AppointmentPayments>(appointmentPaymentModel);
                        appointmentPayment.OrganizationId = token.OrganizationID;
                        appointmentPayment.CommissionPercentage = organization.BookingCommision;
                        //appointmentPayment.CreatedBy = token.UserID;
                        appointmentPayment.CreatedBy = patient.UserID;
                        appointmentPayment.CreatedDate = DateTime.UtcNow;
                        _appointmentPaymentRepository.Create(appointmentPayment);
                        _appointmentPaymentRepository.SaveChanges();
        ...
        public JsonModel Updatestatusforappointment(PatientAppointmentModel patientAppointmentModel, TokenModel token)
        {
            PatientAppointment patientAppointment = null;
            //Patients patient = null;
            ...
            if (!ReferenceEquals(patientAppointment, null))
            {
                ...
                SQLResponseModel sqlResponse = _appointmentRepository.UpdateApptStatusFromPaymentpage<SQLResponseModel>(patientAppointmentModel.PatientAppointmentId).FirstOrDefault();
                return new JsonModel()
                {
                    data = new object(),
                    StatusCode = sqlResponse.StatusCode,
        ...
        public JsonModel DeleteAppointmentfrompaymentpage(int appointmentId, int? parentAppointmentId, bool deleteSeries, bool isAdmin, TokenModel token)
        {
            try
            {
                List<AppointmentAuthModel> list = null;
                ...
                if (appointmentId > 0)
                {
                    SQLResponseModel sqlResponse = _appointmentRepository.DeleteAppointmentfrompaymentpage<SQLResponseModel>(appointmentId, isAdmin, deleteSeries, token).FirstOrDefault();
                    return new JsonModel()
                    {
                        data = new object(),
                        StatusCode = sqlResponse.StatusCode,
        ...
        public JsonModel CheckAppointmentTimeExpiry(int appointmentId, TokenModel token)
        {
            try
            {
                ...
                if (appointmentId > 0)
                {
                    ...
                    patientAppointment = _context.PatientAppointment.Where(x => x.Id == appointmentId && x.IsActive == true && x.IsDeleted == false).FirstOrDefault();
                    if (patientAppointment != null)
                    {
                        var diffHours = (patientAppointment.CreatedDate.Value - DateTime.Now).TotalHours;
                        if (diffHours > 24)
                        {
                            DeleteAppointmentfrompaymentpage(appointmentId, null, false, false, token);
                            return response = new JsonModel()
                            {
                                data = new object(),
                        ...
                        else
                        {
                            ...
                            {
                                data = new object(),
                                Message = "Appointment Time Not Expired",
        ...
        public JsonModel BookUrgentCareAppointment(PatientAppointmentModel patientAppointmentModel, TokenModel token)
        {
            JsonModel response = SaveAppointmentFromPatientPortal(patientAppointmentModel, token);
            if (response.StatusCode == (int)HttpStatusCodes.OK)
            {
                ...
                AppointmentStatusModel.Id = appointmentId;
                AppointmentStatusModel.Status = patientAppointmentModel.StatusId == null ? "APPROVED" : "PENDING";
                ApproveUrgentCareAppointment(AppointmentStatusModel, token);
        ...
        public JsonModel ApproveUrgentCareAppointment(AppointmentStatusModel appointmentStatusModel, TokenModel token)
        {
            response = new JsonModel(new object(), StatusMessage.ServerError, (int)HttpStatusCodes.InternalServerError, string.Empty);
            List<AppointmentAuthModel> list = null;
            PatientAppointment patientAppointment = null;
        ...
        public JsonModel UrgentCareRefundAppointmentFee(int appointmentId, TokenModel token)
        {
            try
            {
                var org = (OrganizationDetailModel)_organizationService.GetOrganizationDetailsById(token).data;
                var result = DeleteUrgentCareAppointment(appointmentId, token);
                var appPayment = _appointmentPaymentRepository.Get(p => p.AppointmentId == appointmentId);
                if (appPayment != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(appPayment.PaymentToken))
                    {
                        ...
                        options.Amount = Convert.ToInt64(appPayment.BookingAmount);
        ...
        public int GetStaffUserId(int appointmentId)
        {
            int staffuserid;
            AppointmentStaff appointmentStaff = null;
            ...
            if (appointmentId > 0)
            {
                ...
                appointmentStaff = _context.AppointmentStaff.Where(a => a.PatientAppointmentID == appointmentId).FirstOrDefault();
                staffdetails = _context.Staffs.Where(a => a.Id == appointmentStaff.StaffID).FirstOrDefault();


            }
            staffuserid = staffdetails.UserID;
           
            return staffuserid;
        }

        public int GetPatientUserId(int appointmentId)
        {
            int patientuserid;
            PatientAppointment pateintapptdetails = null;
            Patients patientdetails = null;
            if (appointmentId > 0)
            {

                pateintapptdetails = _context.PatientAppointment.Where(a => a.Id == appointmentId).FirstOrDefault();
                patientdetails = _context.Patients.Where(a => a.Id == pateintapptdetails.PatientID).FirstOrDefault();


            }
            patientuserid = (int)patientdetails.UserID;
           
            return patientuserid;
        }

        public JsonModel GetLastUrgentCareCallStatus(int providerId, TokenModel token)
        {
            response = new JsonModel();
            try
            {
                ...
                Staffs staffdetails = null;
                List<PatientAppointment> appts = new List<PatientAppointment>();
                staffdetails = _context.Staffs.Where(a => a.UserID == providerId).FirstOrDefault();
                appts = _patientAppointmentRepository.GetLastUrgentCareCallStatus(staffdetails.Id, token);
                
                var latestAppt = appts.OrderByDescending(x => x.CreatedDate).FirstOrDefault();
                if (latestAppt != null)
                {
                    var diffMins = (DateTime.UtcNow - latestAppt.CreatedDate.Value).TotalMinutes;
                    if (diffMins < 0 || diffMins > 10)
                    {
                        istimeexpired = true;
                    }
                }
                response.data = istimeexpired ? null : latestAppt;
                response.Message = StatusMessage.FetchMessage;
                response.StatusCode = (int)HttpStatusCode.OK;
        ...
        public JsonModel GetLastUrgentCareCallStatusForPatientPortal(int patientuserid, TokenModel token)
        {
            response = new JsonModel();
            try
            {
                ...
                Patients patientdetails = null;
                List<PatientAppointment> appts = new List<PatientAppointment>();
                patientdetails = _context.Patients.Where(a => a.UserID == patientuserid).FirstOrDefault();
                //appts = _patientAppointmentRepository.GetLastUrgentCareCallStatus(staffdetails.Id, token);
                appts = _context.PatientAppointment.Where(x => x.PatientID == patientdetails.Id && x.IsUrgentCareAppointment == true && x.IsActive==true && x.IsDeleted==false).ToList();
        ...
        public JsonModel DeleteUrgentCareAppointment(int appointmentId, TokenModel token)
        {
            PatientAppointment patientAppointment = _appointmentRepository.GetByID(appointmentId);
            //List<AppointmentAuthModel> list = _patientRepository.GetAuthDataForPatientAppointment<AppointmentAuthModel>((int)patientAppointment.PatientID, (int)patientAppointment.AppointmentTypeID, patientAppointment.StartDateTime, patientAppointment.EndDateTime, InsurancePlanType.Primary.ToString(), appointmentId, isAdmin, patientAppointment.PatientInsuranceId, patientAppointment.AuthorizationId).Where(x => x.AuthProcedureCPTLinkId != null && x.AuthProcedureCPTLinkId > 0).ToList();
            //if (list != null && list.Count > 0 && list.First().AuthorizationMessage.ToLower() != "valid")
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                ...
                {
                    dbAppointments = _appointmentRepository.Get(a => a.Id == appointmentId);
                    dbAppointments.IsActive = false;
                    dbAppointments.IsDeleted = true;
                    dbAppointments.UpdatedBy = token.UserID;
                    dbAppointments.UpdatedDate = DateTime.UtcNow;
                    _appointmentRepository.Update(dbAppointments);
                    _appointmentRepository.SaveChanges();
                    ...
                    {
                        data = new object(),
                        Message = StatusMessage.DeletedUrgentCareAppointment,
                        StatusCode = (int)HttpStatusCodes.OK
                    };
        ...
        public JsonModel GetLastPatientFollowupDetails(int patientuserid, TokenModel token)
        {
            response = new JsonModel();
            try
            {
                ...
                List<PatientAppointment> appts = new List<PatientAppointment>();
                //patientdetails = _context.Patients.Where(a => a./*UserID*/ == patientuserid).FirstOrDefault();
                appts = _context.PatientAppointment.Where(x => x.PatientID == patientuserid
                && x.IsActive == true && x.IsDeleted == false ).OrderByDescending(x=>x.CreatedDate).ToList();

                var latestAppt = appts.FirstOrDefault();
                if (latestAppt != null)
                {
                    var diffMins = (DateTime.UtcNow - latestAppt.StartDateTime).Days;
                    if ( diffMins > 14)
                    {
                        istimeexpired = true;
                    }
        ...
        public JsonModel GetFilteredAppointmentList(string locationIds, string staffIds, string patientIds, DateTime? fromDate, DateTime? toDate, string patientTags, string staffTags, string appttype, string apptmode,TokenModel token)
        {
            try
            {
                List<PatientAppointmentModel> list = new List<PatientAppointmentModel>();
                if (!string.IsNullOrEmpty(locationIds) && (!string.IsNullOrEmpty(staffIds) || !string.IsNullOrEmpty(patientIds)))
                {
                    list = _appointmentRepository.GetFilteredAppointmentList<PatientAppointmentModel>(locationIds, staffIds, patientIds, fromDate, toDate, patientTags, staffTags,  appttype,  apptmode, token.OrganizationID).ToList();
                    var hostingServer = _configuration.GetSection("HostingServer").Value;
                    list.ForEach(x =>
                    {
                        LocationModel locationModal = _locationService.GetLocationOffsets(x.ServiceLocationID, token);//GetLocationOffsets(x.ServiceLocationID);
        ...
        public JsonModel GetUrgentCareAppointmentList(PatientAppointmentFilterModel appointmentFilterModel, TokenModel token)
        {
            List<PendingAppointmentViewModel> list = new List<PendingAppointmentViewModel>();

            list = _appointmentRepository.GetUrgentCareAppointmentList<PendingAppointmentViewModel>(appointmentFilterModel, token).ToList();
            var hostingServer = _configuration.GetSection("HostingServer").Value;
            if (list != null && list.Count() > 0)
            {
                foreach (var x in list)
        ...
        public JsonModel GetFilteredAppointmentsForReporting(AppointmentReportingFilterModel appointmentReportingFilter, TokenModel token)
        {
            XElement programTypes = null, xmlEnrolledPrograms = null;
            string[] programs = appointmentReportingFilter.ProgramTypes?.Split(",");
            string[] enrolledPrograms = appointmentReportingFilter.EnrolledPrograms?.Split(",");
            if (programs != null && programs.Length > 0)
            {
                //create xml element
                programTypes = new XElement("Parent", programs.Select((x, index) => new XElement("Child",
                            new XElement("Id", x))));
            }
            if (enrolledPrograms != null && enrolledPrograms.Length > 0)
            {
                xmlEnrolledPrograms = new XElement("Parent", enrolledPrograms.Select((x, index) => new XElement("Child",
                            new XElement("Id", x))));
            }
            appointmentReportingFilter.XmlProgramTypes = programTypes?.ToString();
            appointmentReportingFilter.XmlEnrolledPrograms = xmlEnrolledPrograms?.ToString();

            AppointmentReportingResponseModel appointmentReportingResponses = _appointmentRepository.GetFilteredAppointmentsForReporting(appointmentReportingFilter, token);
            appointmentReportingResponses?.AppointmentReportingList.ForEach(x =>
            {
                x.StartDateTime = CommonMethods.ConvertFromUtcTimeWithOffset(x.StartDateTime, CommonMethods.ConvertOffsetToMinutes(x.DaylightSavingTime), CommonMethods.ConvertOffsetToMinutes(x.StandardTime), x.TimeZoneName, token);
                x.EndDateTime = CommonMethods.ConvertFromUtcTimeWithOffset(x.EndDateTime, CommonMethods.ConvertOffsetToMinutes(x.DaylightSavingTime), CommonMethods.ConvertOffsetToMinutes(x.StandardTime), x.TimeZoneName, token);

                x.Programs = !string.IsNullOrEmpty(x.XmlProgramModel) ? XDocument.Parse(x.XmlProgramModel).Descendants("Child").Select(y => new AppointmentReportingProgramModel()
                {
                    ProgramName = y.Element("ProgramName").Value,
                }).ToList() : new List<AppointmentReportingProgramModel>(); x.XmlProgramModel = null;

                x.CareManagers = !string.IsNullOrEmpty(x.XmlCareMangerModel) ? XDocument.Parse(x.XmlCareMangerModel).Descendants("Child").Select(y => new AppointmentReportingCareManagerModel()
                {
                    Id = Convert.ToInt32(y.Element("Id").Value),
                    CareManagerName = y.Element("CareManagerName").Value,
                }).ToList() : new List<AppointmentReportingCareManagerModel>(); x.XmlCareMangerModel = null;
            });

            response = new JsonModel(appointmentReportingResponses, StatusMessage.FetchMessage, (int)HttpStatusCodes.OK);
            response.meta = new Meta(appointmentReportingResponses?.AppointmentReportingList, appointmentReportingFilter);
            return response;
        }
