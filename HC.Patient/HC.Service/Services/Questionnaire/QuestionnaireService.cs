    ...
    public class QuestionnaireService : BaseService, IQuestionnaireService
    {
        ...
        public JsonModel SavePatientDocumentAnswer(AnswersModel answersModel, TokenModel tokenModel)
        {
            ...
            if (answersModel.PatientID > 0 && answersModel.DocumentId > 0 && answersModel.Answer != null && answersModel.Answer.Count > 0)
            {
                ...
                saveHRALogs(answersModel.Logs, tokenModel);
                if(answerResponse != null && answerResponse.StatusCode == 200)
                {
                    ...
                    if (user != null && user.UserRoles != null && user.UserRoles.RoleName == OrganizationRoles.Patient.ToString())
                    {
                        ...
                        bool isSuccess = _patientAlertService.SavePatientAlerts("One Assessment Completed", answersModel.PatientID, masterAlertTypeId, null, tokenModel);
                    }
                }
        ...
        public void saveHRALogs(LogModel logModel, TokenModel tokenModel)
        {
            ...
            entity.ReportType = "HRA";
            entity.ReportTypeId = Convert.ToInt32(CommonEnum.LogReportType.HRA);
