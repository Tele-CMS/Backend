...
namespace HC.Patient.Repositories.Repositories.Patient
{
    public class PatientRepository : RepositoryBase<Patients>, IPatientRepository
    {
        ...
        public IQueryable<T> GetPatientDiagnosisCodes<T>(int patientId, FilterModel filterModel) where T : class, new()
        {
        ...
        /// <returns></returns>
        /// New method with checking existing username and email 
        public int ImportPatientCCDA(PatientClinicalInformation patientClinicalInformation, int organizationID, int userID)
        {
            try
            {
        ...
        public IQueryable<T> GetRedAlertsInfo<T>(AlertsRedIndicatorFilterModel filterModel, TokenModel token) where T : class, new()
        {
            SqlParameter[] parameters = {
                                          //new SqlParameter("@PatientId", filterModel.PatientId),
                                          new SqlParameter("@OrganizationId", token.OrganizationID),
                                          new SqlParameter("@AlertTypeIds", filterModel.AlertTypeIds),
                                          ...
                                          new SqlParameter("@DOB", filterModel.DOB),
                                          new SqlParameter("@MedicalID", filterModel.MedicalID),
                                          new SqlParameter("@EligibilityStatus", filterModel.EligibilityStatus),
                                          new SqlParameter("@StartAge", filterModel.StartAge),
                                          new SqlParameter("@EndAge", filterModel.EndAge),
                                          new SqlParameter("@RiskIds", filterModel.RiskIds),
                                          new SqlParameter("@ProgramIds", filterModel.ProgramIds),
                                          new SqlParameter("@GenderIds", filterModel.GenderIds),
                                          new SqlParameter("@RelationshipIds", filterModel.RelationshipIds),
                                          new SqlParameter("@PrimaryConditionId", filterModel.PrimaryConditionId),
                                          new SqlParameter("@ComorbidConditionIds", filterModel.ComorbidConditionIds),
                                          new SqlParameter("@PageNumber", filterModel.pageNumber),
                                          new SqlParameter("@PageSize", filterModel.pageSize),
            ...
            return _context.ExecStoredProcedureListWithOutput<T>(SQLObjects.ADM_GetRedAlertsInfo.ToString(), parameters.Length, parameters).AsQueryable();
        }
        #endregion
        ...
        private Patients SavePatientDemographics(int organizationID, int userID, PatientClinicalInformation patientClinicalInformation)
        {
            try
            {
                ...
                var MRN = "MRN-" + (random.Next(111111, 999999).ToString());
                PHIEncryptedModel pHIEncryptedModel = GetEncryptedPHIData<PHIEncryptedModel>(patientClinicalInformation.ptDemographicDetail.FirstName, null, patientClinicalInformation.ptDemographicDetail.LastName, DOB.ToString("yyyy-MM-dd HH:mm:ss.fffffff"), patientClinicalInformation.ptDemographicDetail.Email, patientClinicalInformation.ptDemographicDetail.SSN, MRN, null, null, null, null, null, null, null).FirstOrDefault();
        ...
        public PatientInfoDetails GetPatientsDashboardDetails(int PatientID, TokenModel token)
        {
            SqlParameter[] parameters = { new SqlParameter("@patientId", PatientID),
            new SqlParameter("@OrganizationId", token.OrganizationID)};
            return _context.ExecStoredProcedureListWithOutputForPatientInfo(SQLObjects.PAT_GetPatientDashbaordDetails.ToString(), parameters.Length, parameters);
        }

        public IQueryable<T> UpdatePatientBBData<T>(string dOB, string genderName, string identifier, int patientId, string firstName, string lastName, string middleName, string address, string city, string zip, string stateName, string phoneNumber, string phoneNumberType, TokenModel token) where T : class, new()
        {
            SqlParameter[] parameters = { new SqlParameter("@DOB", Convert.ToDateTime(dOB)),
                                          new SqlParameter("@Gender", genderName),
                                          new SqlParameter("@Identifier", identifier),
                                          new SqlParameter("@PatientId", patientId),
                                          new SqlParameter("@OrganizationId",token.OrganizationID),
                                          new SqlParameter("@PatientLocation", token.LocationID),
                                          ...
                                          new SqlParameter("@PhoneNumber", phoneNumber),
                                          new SqlParameter("@PhoneNumberType", phoneNumberType)
            };
             return _context.ExecStoredProcedureListWithOutput<T>(SQLObjects.PAT_UpdateProfileDetails, parameters.Length, parameters).AsQueryable(); 
        }
